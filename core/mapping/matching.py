...
def ransac_filter(points_10x: np.ndarray, points_40x: np.ndarray,
                 num_iterations: int = 100, sample_size: int = 10,
                 inlier_threshold: float = 20.0,
                 min_inliers_ratio: float = 0.5) -> Tuple[np.ndarray, np.ndarray]:
...
def iterative_matching(points_10x: np.ndarray, points_40x: np.ndarray,
    ...
    for iteration in range(max_iterations):
        ...
        if len(matched_10x) < 10:
            raise ValueError("Too few matches found")
