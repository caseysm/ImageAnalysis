...
class ND2Segmentation:
    ...
    def load_nd2_images(self):
        ...
        try:
            # Open the ND2 file using the nd2 package
            with nd2.ND2File(self.input_file) as ndfile:
                # Get image dimensions and metadata
                metadata = ndfile.metadata
                logger.info(f"ND2 file metadata: {metadata}")

                # Load full data
                full_data = ndfile.asarray()
                logger.info(f"Loaded ND2 data with shape {full_data.shape}")

                # Get the number of channels (based on the shape of the data)
                # ND2 arrays are typically organized as (t, z, y, x, c) or (y, x, c)
                if len(full_data.shape) == 5:  # (t, z, y, x, c)
                    num_channels = full_data.shape[4]
                    # Get first timepoint, first z-slice
                    nuclear_image = full_data[0, 0, :, :, self.nuclear_channel]
                    cell_image = full_data[0, 0, :, :, self.cell_channel]
                elif len(full_data.shape) == 4:  # (z, y, x, c) or (t, y, x, c)
                    num_channels = full_data.shape[3]
                    # Get first frame/z-slice
                    nuclear_image = full_data[0, :, :, self.nuclear_channel]
                    cell_image = full_data[0, :, :, self.cell_channel]
                elif len(full_data.shape) == 3:  # (y, x, c)
                    num_channels = full_data.shape[2]
                    nuclear_image = full_data[:, :, self.nuclear_channel]
                    cell_image = full_data[:, :, self.cell_channel]
                else:
                    raise ValueError(f"Unexpected data shape: {full_data.shape}")

                logger.info(f"Number of channels: {num_channels}")

                if num_channels <= max(self.nuclear_channel, self.cell_channel):
                    raise ValueError(f"Not enough channels in ND2 file. "
                                     f"Asked for channels {self.nuclear_channel} and {self.cell_channel}, "
                                     f"but file only has {num_channels} channels.")

                logger.info(f"Loaded nuclear channel with shape {nuclear_image.shape}")
                logger.info(f"Loaded cell channel with shape {cell_image.shape}")
